#!/usr/bin/env node
/**
 * Re-TTS + Whisper every unique v286 ALL residual fail.
 *
 *   node scripts/listen_v286_residual_recheck.mjs
 *   node scripts/listen_v286_residual_recheck.mjs --force-tts
 *   node scripts/listen_v286_residual_recheck.mjs --whisper=small
 *   node scripts/listen_v286_residual_recheck.mjs --round=2
 */
...
const outDir = join(root, `extracted/listen_v286_residual/r${ROUND}`);
const mp3Dir = join(outDir, 'mp3');
const whisperDir = join(outDir, 'whisper_work');
const uniquePath = join(root, 'extracted/v286_residual_unique.json');
const reportPath = join(root, 'extracted/listen_stt_v286_residual_recheck.json');
...
async function main() {
  ...
  const items = (uniq.items || []).map((it, idx) => {
    ...
    const hash = createHash('sha1').update(`v286r${ROUND}|${fish}`).digest('hex').slice(0, 16);
    return {
      id: `u${idx}_${hash}`,
      bare: it.bare,
      spoken: it.spoken,
      fish_v286: it.fish_v286 || it.fishOld,
      count: it.count,
      books: it.books,
      fish,
      hash,
      prepChanged: fish !== (it.fish_v286 || it.fishOld),
    };
  });

  console.log(`v286 residual unique=${items.length} round=${ROUND} live=`, liveVersion);
  ...
  const rows = await mapPool(items, CONCURRENCY, async (row, idx) => {
    ...
    console.log(`[tts ${idx + 1}/${items.length}] ${row.bare} ${ok ? size : 'FAIL ' + status}`);
    await sleep(PAUSE_MS);
    return { ...row, mp3: ok ? file : null, ttsOk: ok, size, cached: false };
  });
  ...
  const scored = rows.map((r) => {
    ...
    const j = r.ttsOk
      ? judge(r.fish, transcript)
      : { pass: false, overlap: 0, ratio: 0, missing: [], flags: [{ kind: 'tts_fail' }] };
    return { ...r, transcript, pass: j.pass, overlap: j.overlap, ratio: j.ratio, missing: j.missing, flags: j.flags };
  });

  const pass = scored.filter((x) => x.pass).length;
  const fail = scored.filter((x) => !x.pass);
  const beforeClips = 29 + 8 + 20;
  const report = {
    ...
    ttsCache: 'v59',
    sourceReports: [
      'extracted/listen_stt_tawheed_ALL_v286.json',
      'extracted/listen_stt_usool_ALL_v286.json',
      'extracted/listen_stt_nawawi_ALL_v286.json',
    ],
    before: {
      clipsFail: beforeClips,
      uniqueBare: scored.length,
      byBook: { tawheed: 29, usool: 8, nawawi: 20 },
    },
    after: {
    ...
    notes: [
      'Display text unchanged; speech carriers strengthened (أعني/إنّه/قل؛ avoid هو→English who)',
      'Re-TTS via live /api/tts with local prepareFishTtsText; Whisper small',
    ],
  };
  writeFileSync(reportPath, JSON.stringify(report, null, 2));
  writeFileSync(join(outDir, 'score.json'), JSON.stringify({ pass, fail: fail.length, fails: fail, all: scored }, null, 2));
  console.log(JSON.stringify(report.after, null, 2));
  console.log(`Wrote ${reportPath}`);
  for (const x of fail) {
    console.log(JSON.stringify({ bare: x.bare, fish: x.fish, stt: String(x.transcript).slice(0, 60), ov: x.overlap }));
  }
}
