...
const sttCacheDir = join(audioRoot, 'stt');
const reportPath = join(root, 'extracted/listen_stt_ALL_questions.json');
const clipsIndexPath = join(audioRoot, 'clips_index.json');
...
async function materializeClipMp3s(clips, report) {
  const workDir = join(audioRoot, 'whisper_work');
