#!/usr/bin/env node
/**
 * Nawawi-only full listen/STT (v297) — Fish «راوٍ عربي حكيم» — EVERY q + all options.
 *
 *   node scripts/listen_stt_nawawi_hakim_v297.mjs --phase=both
 *   node scripts/listen_stt_nawawi_hakim_v297.mjs --compare-only
 *
 * prepareTtsPayload → prepareFishTtsText → POST /api/tts.
 * Verifies X-TTS-Provider=fish, X-TTS-Voice=aa9c8260…, X-TTS-Voice-Name=raawi-arabi-hakim.
 *
 * MP3 cache: extracted/listen_all/mp3_hakim/<hash>.mp3
 * Progress:  extracted/listen_stt_nawawi_ALL_hakim_v297.json
 */
...
const BOOK_FILTER = (process.argv.find((a) => a.startsWith('--book='))?.slice(7) || 'nawawi')
  .toLowerCase()
  .trim();
const HAKIM_VOICE_ID = 'aa9c8260269c411d9863ab1b1bfa3158';
const HAKIM_VOICE_NAME = 'raawi-arabi-hakim';
...
const mp3Dir = join(audioRoot, 'mp3_hakim');
const sttCacheDir = join(audioRoot, 'stt_nawawi_hakim');
const reportArg = process.argv.find((a) => a.startsWith('--report='))?.slice(9);
const reportPath = reportArg
  ? join(root, reportArg.replace(/^\.\//, ''))
  : join(root, 'extracted/listen_stt_nawawi_ALL_hakim_v297.json');
const clipsIndexPath = join(audioRoot, 'clips_index_nawawi_hakim.json');
const summaryPath = join(root, 'extracted/listen_stt_nawawi_ALL_hakim_v297_summary.json');
...
function textHash(text) {
  // Namespace hashes so Hakim clips never collide with Hamed/legacy Fish caches.
  return createHash('sha256')
    .update(`hakim|${HAKIM_VOICE_ID}|${String(text || '')}`, 'utf8')
    .digest('hex')
    .slice(0, 16);
}
...
function prepareField(q, field, spoken, { stripAyah = false } = {}) {
  ...
  return { spoken: String(spoken || ''), prepared: prepared || '', fish, ttsText: fish };
}
...
function judgeClip(clip, transcript) {
  ...
  for (const w of hw) {
    if (w.length < 3) continue;
    if (!iw.includes(w) && !intended.includes(w) && !compactLetters(clip.fish).includes(w)) extra.push(w);
  }
  ...
  for (const w of hw) {
    ...
    if (best < 0.35 && !intended.includes(w) && !compactLetters(clip.fish).includes(w)) mangled.push(w);
  }
  ...
  const sttNoiseOnly = overlap >= 0.88 && ratio >= 0.8 && !flags.some((f) => f.kind === 'allah_misread');

  const hardKinds = new Set(['missing_words', 'mangled', 'allah_misread']);
  const hard = flags.filter((f) => hardKinds.has(f.kind));
  const pass = sttNoiseOnly || (hard.length === 0 && (il < 12 || ratio >= 0.5));
  ...
  if (!pass && clip.prepared && bareLetters(clip.prepared) === intended) {
    flags.push({
      ...
      detail: 'prepared letters correct; STT/Fish حكيم diverge',
    });
  }
...
async function fetchTts(text) {
  const res = await fetch(`${base.replace(/\/$/, '')}/api/tts`, {
    ...
    body: JSON.stringify({ text, voice: HAKIM_VOICE_ID }),
  });
  const provider = (res.headers.get('x-tts-provider') || '').toLowerCase();
  const voice = res.headers.get('x-tts-voice') || '';
  const voiceName = (res.headers.get('x-tts-voice-name') || '').toLowerCase();
  const buf = Buffer.from(await res.arrayBuffer());
  if (res.status === 200) {
    if (provider !== 'fish') {
      throw new Error(`Expected X-TTS-Provider=fish, got "${provider}"`);
    }
    if (voice !== HAKIM_VOICE_ID) {
      throw new Error(`Expected X-TTS-Voice=${HAKIM_VOICE_ID}, got "${voice}"`);
    }
    if (voiceName && voiceName !== HAKIM_VOICE_NAME) {
      throw new Error(`Expected X-TTS-Voice-Name=${HAKIM_VOICE_NAME}, got "${voiceName}"`);
    }
  }
  return { status: res.status, buf, size: buf.length, provider, voice, voiceName };
}
...
async function ensureMp3(clip) {
  ...
  if (existsSync(file) && statSync(file).size > 800) {
    return {
      ...
      cached: true,
      provider: 'fish',
      voice: HAKIM_VOICE_ID,
      voiceName: HAKIM_VOICE_NAME,
    };
  }
  if (!clip.fish || letterCount(clip.fish) < 2) {
  ...
  while (attempt < 6) {
    attempt += 1;
    try {
      const ttsText = clip.ttsText || clip.fish;
      const { status, buf, size, provider, voice, voiceName } = await fetchTts(ttsText);
      ...
      if (/Expected X-TTS/.test(String(e?.message || e))) {
        return {
          file: null,
          ttsOk: false,
          ttsStatus: 0,
...
function summarize(report) {
  ...
  const providers = [...new Set(entries.map((c) => c.provider).filter(Boolean))];
  const voices = [...new Set(entries.map((c) => c.voice).filter(Boolean))];
  const voiceNames = [...new Set(entries.map((c) => c.voiceName).filter(Boolean))];
  const hakimOk =
    providers.length > 0 &&
    providers.every((p) => p === 'fish') &&
    voices.every((v) => v === HAKIM_VOICE_ID) &&
    (voiceNames.length === 0 || voiceNames.every((n) => String(n).toLowerCase() === HAKIM_VOICE_NAME));

  report.summary = {
    book: BOOK_FILTER || 'nawawi',
    label: 'الأربعون النووية',
    mode: 'listen_stt_nawawi_ALL_hakim_v297',
    note: 'Full nawawi listen: prepareTtsPayload → prepareFishTtsText → Fish راوٍ عربي حكيم via /api/tts + Whisper',
    live: versionInfo().cache || null,
    provider: providers[0] || null,
    voice: voices[0] || null,
    voiceName: voiceNames[0] || null,
    voiceNameAr: 'راوٍ عربي حكيم',
    hakimConfirmed: hakimOk,
    fishVoiceId: HAKIM_VOICE_ID,
    questionsTotal: all.length,
    expectedClips: entries.length || null,
    ...
    totalCompleted: judgedQ.length + judgedOpt.length,
    totalPass: judgedQ.filter((c) => c.pass).length + judgedOpt.filter((c) => c.pass).length,
    totalFail: failQ.length + failOpt.length,
    brokenExamples: broken,
    version: versionInfo(),
    whisperModel: report.whisperModel || WHISPER_MODEL,
  };
  report.provider = providers[0] || 'fish';
  report.voice = voices[0] || HAKIM_VOICE_ID;
  report.voiceName = voiceNames[0] || HAKIM_VOICE_NAME;
  report.hakimConfirmed = hakimOk;
  report.mode = 'listen_stt_nawawi_ALL_hakim_v297';
  report.phase = {
  ...
  await runPool(clips, CONCURRENCY, async (clip, i) => {
    ...
    if (!FORCE_TTS && prev?.ttsOk && prev.hash === clip.hash && prev.mp3 && existsSync(prev.mp3)) {
      ...
      report.clips[clip.id] = {
        ...
        fish: clip.fish,
        ttsText: clip.ttsText || clip.fish,
        provider: prev.provider || 'fish',
        voice: prev.voice || HAKIM_VOICE_ID,
        voiceName: prev.voiceName || HAKIM_VOICE_NAME,
      };
      return;
    }
    ...
    const entry = {
      ...
      err: res.err || null,
      provider: res.provider || (res.cached ? 'fish' : null),
      voice: res.voice || (res.cached ? HAKIM_VOICE_ID : null),
      voiceName: res.voiceName || (res.cached ? HAKIM_VOICE_NAME : null),
    };
    if (res.skip) {
      entry.pass = true;
      entry.skipped = true;
...
async function materializeClipMp3s(clips, report) {
  const workDir = join(audioRoot, 'whisper_work_nawawi_hakim');
...
async function sttPhase(clips, report) {
  ...
  const { workDir, n } = await materializeClipMp3s(
    need.length ? need : clips.filter((c) => report.clips[c.id]?.ttsOk && !report.clips[c.id]?.skip),
    report
  );
          ...
          if (
            ...
            entry.flags = [
              ...(entry.flags || []),
              {
                ...
                detail: `prep letters correct; ${WHISPER_MODEL}+${WHISPER_FAIL_MODEL} mangle — Fish حكيم/STT limitation`,
              },
            ];
          }
        }
...
console.log(
  `Hakim: provider=${report.provider} voice=${report.voice} voiceName=${report.voiceName} confirmed=${report.hakimConfirmed}`
);
