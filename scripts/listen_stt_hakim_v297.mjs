#!/usr/bin/env node
/**
 * Full-bank listen/STT via live Fish «راوٍ عربي حكيم» (v297).
 *
 *   node scripts/listen_stt_hakim_v297.mjs --book=usool --phase=both
 *
 * Uses prepareTtsPayload → prepareFishTtsText → POST /api/tts
 * Verifies X-TTS-Provider=fish, X-TTS-Voice=aa9c8260…, X-TTS-Voice-Name=raawi-arabi-hakim.
 * MP3 cache: extracted/listen_all/mp3_hakim/<hash16>.mp3
 * Report:    extracted/listen_stt_<book>_ALL_hakim_v297.json
 */
...
const PAUSE_MS = Number(process.argv.find((a) => a.startsWith('--pause='))?.slice(8) || 300);
const WHISPER_MODEL =
...
const bookTag = BOOK_FILTER ? `${BOOK_FILTER}_hakim` : 'all_hakim';
...
const reportPath = reportArg
  ...
  : BOOK_FILTER
    ? join(root, `extracted/listen_stt_${BOOK_FILTER}_ALL_hakim_v297.json`)
    : join(root, 'extracted/listen_stt_ALL_hakim_v297.json');
  ...
  for (const q of all) {
    ...
    if (phase === 'q' || phase === 'both') {
      ...
      clips.push({
        ...
        book: q.book,
        ...prep,
        hash: textHash(`hakim|${prep.ttsText || prep.spoken || q.id}`),
      });
    }
      ...
      if (q.type === 'mc' && Array.isArray(aq.a)) {
        ...
        aq.a.forEach((opt, i) => {
          ...
          clips.push({
            ...
            book: q.book,
            ...prep,
            hash: textHash(`hakim|${prep.ttsText || prep.spoken || `${q.id}:a${i}`}`),
          });
        });
      } else if (q.type === 'tf') {
        for (const [field, text] of [
          ...
          clips.push({
            ...
            book: q.book,
            ...prep,
            hash: textHash(`hakim|${prep.ttsText || text}`),
          });
        }
      }
    }
...
async function fetchTts(text) {
  ...
  const provider = (res.headers.get('X-TTS-Provider') || res.headers.get('x-tts-provider') || '').toLowerCase();
  const voice = res.headers.get('X-TTS-Voice') || res.headers.get('x-tts-voice') || '';
  const voiceName =
    res.headers.get('X-TTS-Voice-Name') || res.headers.get('x-tts-voice-name') || '';
  const quality = res.headers.get('X-TTS-Quality') || res.headers.get('x-tts-quality') || '';
  if (res.status === 200) {
    if (provider !== 'fish') {
...
async function ensureMp3(clip) {
  ...
  if (existsSync(file) && statSync(file).size > 800 && !FORCE_TTS) {
    return {
      file,
      ttsOk: true,
      ttsStatus: 200,
  ...
  const payload = clip.ttsText || clip.fish;
  if (!payload || letterCount(payload) < 2) {
  ...
  while (attempt < 6) {
    attempt += 1;
    try {
      const { status, buf, size, provider, voice, voiceName, quality } = await fetchTts(payload);
...
function summarize(report) {
  ...
  for (const c of entries) {
    if (c.ttsProvider) providers[c.ttsProvider] = (providers[c.ttsProvider] || 0) + 1;
    if (c.ttsVoice) voices[c.ttsVoice] = (voices[c.ttsVoice] || 0) + 1;
    if (c.ttsVoiceName) voiceNames[c.ttsVoiceName] = (voiceNames[c.ttsVoiceName] || 0) + 1;
  }

  report.provider = 'fish';
  report.voice = HAKIM_VOICE_ID;
  report.voiceName = 'راوٍ عربي حكيم';
  report.note =
    'Full usool listen: prepareTtsPayload → prepareFishTtsText → Fish راوٍ عربي حكيم via /api/tts + Whisper';
  report.summary = {
    ...
    ttsProviderCounts: providers,
    ttsVoiceCounts: voices,
    ttsVoiceNameCounts: voiceNames,
    confirmedFishHakim:
      Object.keys(voices).length > 0 &&
      Object.keys(voices).every((v) => v === HAKIM_VOICE_ID) &&
      Object.keys(providers).every((p) => p === 'fish') &&
      (Object.keys(voiceNames).length === 0 ||
        Object.keys(voiceNames).every((n) => n === HAKIM_VOICE_NAME)),
  };
  report.phase = {
...
async function ttsPhase(clips, report) {
  ...
  await runPool(clips, CONCURRENCY, async (clip, i) => {
    ...
    const entry = {
      ...
      err: res.err || null,
      ttsProvider: res.provider || null,
      ttsVoice: res.voice || null,
      ttsVoiceName: res.voiceName || null,
      ttsQuality: res.quality || null,
    };
    if (res.skip) {
      entry.pass = true;
      entry.skipped = true;
    ...
    if (fn > 0) {
      ...
      for (const r of failRaw.results || []) {
        ...
        if (judgment.pass) {
          ...
          // If both models mangle but prepared is correct → Fish/STT limitation
          if (
