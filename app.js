...
function isInventedMetaCitation(s) {
  ...
  if (!t) return false;
  return /هذه من فوائد حديث|الحكم الصحيح هو\s*«|هو المعنى الصحيح المذكور في شرح الكتاب|هو ما ثبت في لفظ الحديث\/الأثر كما أورده الكتاب|الموضع الصحيح الذي استدل به المؤلف في هذا الموطن|هذا ما جاء في حديث معاذ|»\s*هو\s*$/.test(
    t
  );
}
