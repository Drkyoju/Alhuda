...
const ALLAH_NOM = 'اللَّهُ'; // مرفوع
const ALLAH_ACC = 'اللَّهَ'; // منصوب
const ALLAH_GEN = 'اللَّهِ'; // مجرور
const ALLAHUMMA = 'اللَّهُمَّ';
const LILLAH = 'لِلَّهِ';
const BILLAH = 'بِاللَّهِ';
const WALLAH = 'وَاللَّهِ';
const FALLAH = 'فَاللَّهِ';
const TALLAH = 'تَاللَّهِ';
const KALLAH = 'كَاللَّهِ';
const WALILLAH = 'وَلِلَّهِ';
const FALILLAH = 'فَلِلَّهِ';
const ILLA_ALLAH = 'إِلَّا اللَّهَ';
const LA_ILAHA_ILLA_ALLAH = 'لَا إِلَٰهَ إِلَّا اللَّهُ';
const LA_MABUDA_BIHAQQ_ILLA_ALLAH = 'لَا مَعْبُودَ بِحَقٍّ إِلَّا اللَّهَ';
...
const ALLAH_GEN_LAST_RE =
  /^(?:ع(?:ِ)?ن(?:ْ)?د(?:َ)?|م(?:ِ)?ن(?:ْ|َ)?|إِ?ل(?:َ)?ى|الى|ع(?:َ)?ن(?:ْ)?|ع(?:َ)?ل(?:َ)?ى|ف(?:ِ)?ي|م(?:َ)?ع(?:َ)?|ل(?:َ)?د(?:َ)?ى|غ(?:َ)?ي(?:ْ)?ر(?:َ|ِ)?|س(?:ِ)?و(?:َ)?ى|د(?:ُ)?و(?:ْ)?ن(?:َ)?|ح(?:َ)?ق(?:ُّ|ُ|ِ)?|إِ?ف(?:ْ)?ر(?:َ)?اد(?:ُ|ِ)?|افراد|ع(?:ِ)?ب(?:َ)?اد(?:َ)?ة(?:ِ)?|اس(?:ْ)?م(?:ُ|ِ)?|ر(?:َ)?س(?:ُ)?ول(?:ُ|ِ)?|ع(?:َ)?ب(?:ْ)?د(?:ُ|ِ)?|ب(?:ِ)?غ(?:َ)?ي(?:ْ)?ر(?:ِ)?|د(?:ِ)?ين(?:َ|ُ|ِ)?|ش(?:َ)?ر(?:ْ)?ع(?:َ|ُ|ِ)?|ح(?:ُ)?د(?:ُ)?ود(?:َ|ُ|ِ)?|ط(?:َ)?اع(?:َ)?ة(?:ِ)?|م(?:َ)?ع(?:ْ)?ص(?:ِ)?ي(?:َ)?ة(?:ِ)?|لِ?غ(?:َ)?ي(?:ْ)?ر(?:ِ)?|ت(?:َ)?ق(?:ْ)?و(?:َ)?ى|تقوى)$/u;

/** Particles that put الله in the accusative — last word only. */
const ALLAH_ACC_LAST_RE = /^(?:إِ?ن(?:َّ)?|ان|أَ?ن(?:َّ)?|إِ?ل(?:َّ)?ا|اح(?:ْ)?ف(?:َ)?ظ(?:ِ)?|ل(?:َ)?ق(?:ِ)?ي(?:َ)?|يَ?ع(?:ْ)?ب(?:ُ)?د(?:ُ|َ)?|تَ?ع(?:ْ)?ب(?:ُ)?د(?:ُ|َ)?|نَ?ع(?:ْ)?ب(?:ُ)?د(?:ُ|َ)?|يُ?ر(?:َ)?اق(?:ِ)?ب(?:ُ|َ)?|يراقب)$/u;
